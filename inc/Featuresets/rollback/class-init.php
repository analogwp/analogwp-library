...
namespace AnalogWP\CustomLibrary\Featuresets\Rollback;

defined( 'ABSPATH' ) || exit;

use AnalogWP\CustomLibrary\Featuresets\Rollback\Rollbacker;
...
class Init {
	...
	public function __construct() {
		add_filter( 'analog_custom_library_settings_data', array( $this, 'add_rollback_data' ) );
	...
	public function add_rollback_data( $data ) {
		$data['rollback_url']      = wp_nonce_url( admin_url( 'admin-post.php?action=analog_custom_library_rollback&version=VERSION' ), 'analog_custom_library_rollback' );
	...
	public function post_analog_custom_library_rollback() {
		...
		if ( ! $version || ! in_array( $version, $rollback_versions, true ) ) {
			wp_die( esc_html__( 'Error occurred, the version selected is invalid. Try selecting different version.', 'analogwp-library' ) );
		}

		$plugin_slug = 'analogwp-library';
		$plugin_name = AGWP_LIBRARY_PLUGIN_BASE;
		...
		wp_die(
			'',
			esc_html__( 'Rollback to Previous Version', 'analogwp-library' ),
			array(
				'response' => 200,
			)
		);
