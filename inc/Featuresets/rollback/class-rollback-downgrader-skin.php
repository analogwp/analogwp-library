...
class Rollback_Downgrader_Skin extends WP_Upgrader_Skin {
	...
	public function header() {
		...
		echo '<div class="wrap">';
		echo '<h1>' . esc_html__( 'Rolling back plugin…', 'analogwp-library' ) . '</h1>';
		echo '<div class="custom-library-rollback-log">';
	}
	...
	public function footer() {
		...
		echo '</div>'; // .custom-library-rollback-log
		echo '</div>'; // .wrap
	}
	...
	public function after() {
		...
		if ( is_wp_error( $this->result ) || ! $this->result ) {
			echo '<div class="notice notice-error">';
			echo '<p>' . esc_html__( 'Rollback failed during installation.', 'analogwp-library' ) . '</p>';
			...
			echo '<p>' . esc_html__( 'Rollback complete. The plugin files have been restored.', 'analogwp-library' ) . '</p>';
			echo '</div>';
		}

		echo '<p><a class="button button-primary" href="' . esc_url( admin_url( 'plugins.php' ) ) . '">';
		echo esc_html__( 'Go to Plugins page', 'analogwp-library' );
		echo '</a></p>';
	}
}
