 ...
 * be printed on exit.
 */
$results = array('totals' => array('errors' => 0), 'files' => array());
/*
...
if (empty($files_to_scan)) {
    ...
    $results['messages'] = array(array('No files to scan specified'));
    \sysexit($results, 1);
}
/*
...
foreach ($files_to_scan as $file_name) {
    ...
    if (\false === $svg_file) {
        $results['totals']['errors']++;
        $results['files'][$file_name][] = array('errors' => 1, 'messages' => array(array('message' => 'File specified could not be read (' . $file_name . ')', 'line' => null)));
        continue;
    }
    /*
